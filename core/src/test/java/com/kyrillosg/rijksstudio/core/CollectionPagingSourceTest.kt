...
import com.kyrillosg.rijksstudio.core.data.CollectionPagingSource
...
class CollectionPagingSourceTest {
    ...
    private val pagingSource = CollectionPagingSource(
        service = FakeRijksGateway(collectionItems = fakeItems, pageSize = pageSize),
        cache = cacheOf(),
        pageSize = pageSize,
    )
    ...
    fun initialLoad_returnsCorrectLoadResult_whenLoadSizeIsPageSize() = runTest {
        val expected: PagingSource.LoadResult<Int, CollectionItem> = PagingSource.LoadResult.Page(
